...
// ─────────────────────────────────────────────────────────────────────────────
//  START SCHEDULER — exactly 1 announcement per day, rotating through all 5
// ─────────────────────────────────────────────────────────────────────────────
function startAnnouncementSchedulers(client) {
  const DAY_MS = 24 * 60 * 60 * 1000;

  // Rotate through the 5 types — one fires each day
  const sequence = [
    postIntelBriefing,
    postThreatLevel,
    postLoreMessage,
  ...
  // Use day-of-year to pick today's type so it survives restarts consistently
  let index = Math.floor(Date.now() / DAY_MS) % sequence.length;

  // Calculate ms until next midnight UTC so the daily post is predictable
  const now       = new Date();
  ...
  console.log(`📣 First auto-announcement fires in ${Math.round(msUntilMidnight / 3_600_000)}h (at UTC midnight).`);

  // Fire at midnight UTC, then every 24h
  setTimeout(() => {
    // Post today's type
    sequence[index % sequence.length](client);
    index++;
